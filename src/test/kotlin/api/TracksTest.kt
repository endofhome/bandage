...
internal class TracksTest {
    ...
    fun `tracks are available via API`(approver: Approver) {
        ...
        val metadataStorage = StubMetadataStorage(mutableListOf(
            exampleAudioFileMetadata.copy(uuid = uuids[0]),
            exampleAudioFileMetadata.copy(uuid = uuids[1], title = "in a collection", collections = listOf(uuids[2])),
            exampleAudioFileMetadata.copy(uuid = uuids[3],title = "no bitrate", bitRate = null),
            exampleAudioFileMetadata.copy(uuid = uuids[4],title = "no duration", duration = null)
        ))
